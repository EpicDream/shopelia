...
define(['jquery', 'utils'], function($, utils) {
  ...
  that.getElementXPath = function(element) {
    var xpath = '';
    for ( ; element && element.nodeType == 1; element = element.parentNode ) {
      var id = $(element).attr("id");
      if (id && ! that.isRandom(id)) {
  ...
  that.getElementCompleteXPath = function(element) {
    var xpath = '';
    for ( ; element && element.nodeType == 1; element = element.parentNode ) {
      var broAndSis = $(element.parentNode).children(element.tagName);
      var id = $(element).attr("id");
      if (id && (id.length < 15 || ! that.isRandom(id))) {
        xpath = '/'+element.tagName.toLowerCase()+'[@id="'+id+'"]'+xpath;
  ...
  that.getElementsByXPath = function(xpath) { 
  ...
  that.getElementCSSSelectors = function(jelement, complete) {
    var css = '';
    for ( ; jelement && jelement[0].nodeType == 1 ; jelement = jelement.parent() ) {
      var elem_selector = fromParentSelector(jelement, complete);
  ...
  that.getElementsByCSS = function(css) {
    return $(css);
  };

  // Does this xpath lead to a single element.
  that.isXPathUniq = function(xpath) {
    return that.getElementsByXPath(xpath).length == 1;
  };

  // Try to modify xpath to lead to e and only e.
  that.setXPathUniq = function(xpath, e) {
    // Si cours, essai d'ajouter un parent
    // Si long, essai d'ajouter des classes ou des names

    // var elems = that.getElementsByXPath(xpath);
  ...
  that.getElementAttrs = function(e) {
  ...
  that.getLabelAttrs = function(e) {
    var l = that.getInputsLabel(e);
    if (! l) return {};
    return Object({xpath: that.getElementXPath(l), id: l.getAttribute("id"), class: l.getAttribute("class"), text: l.innerText})
  };
  that.getFormAttrs = function(e) {
    var current = e;
    ...
    if (current.tagName.toLowerCase() != "form") 
      ...
      return that.getElementAttrs(current);
  };
  ...
  // h.css
  // h.fullCSS
  // h.attrs : element's attrs. See that.getElementAttrs().
  // h.siblings an Array of siblings' attrs. See that.getElementAttrs().
  // h.parent : parent's attrs. See that.getElementAttrs().
  // h.html : e.outerHTML;
  // For forms elements :
  // h.label : input's label's attributes + xpath. See that.getElementAttrs().
  // h.form : input's form's attributes. See that.getElementAttrs().
  that.getElementContext = function(e) {
    var context = {};
    context.xpath = that.getElementXPath(e);
    context.fullXPath = that.getElementCompleteXPath(e);
    context.css = that.getElementCSSSelectors($(e));
    context.fullCSS = that.getElementCSSSelectors($(e), true);
    context.attrs = that.getElementAttrs(e);
    context.parent = that.getElementAttrs(e.parentElement);
    context.siblings = [];
    var children = e.parentElement.children;
    for (var i = 0 ; i < children.length ; i++)
      context.siblings.push(that.getElementAttrs(children[i]));
    context.html = e.outerHTML;

    var tagName = e.tagName.toLowerCase();
    if (tagName == "input" || tagName == "select" || tagName == "textarea") {
      context.label = that.getLabelAttrs(e);
      context.form = that.getFormAttrs(e);
    }
    return context;
  }
  ...
  that.getInputsLabel = function(e) {
  ...
  that.knowTypes = function(e) {
    var types = [];
    types = types.concat(that.inputs(e));
    types = types.concat(that.links(e));
    types = types.concat(that.labels(e));
    return types;
  };
  // e an input, a select or a textarea,
  // or e include some inputs, selects, textarea.
  that.inputs = function(e) {
    var res = [];
  ...
  that.cookies.add = function(name,value,days,domain) {
    if (days) {
      var date = new Date();
  ...
  that.cookies.get = function(name) {
    var nameEQ = name + "=";
  ...
  that.cookies.remove = function(name) {
    this.add(name,"",-1);
    if (this.get(name))
      this.add(name,"",-1,document.location.host.match(/\.\w+\.\w+$/)[0]);
  };

  that.cookies.keys = function() {
    var res = [];
    var ca = document.cookie.split('; ');
    for(var i=0;i < ca.length;i++) {
  ...
  that.cookies.removeAll = function() {
    var keys = this.keys();
    for (var i in keys)
      this.remove(keys[i]);
  };
