    ...
    %Q{
      ...
      from flinkers as fl
       ...
       inner join (select looks.flinker_id, count(*) as count from looks 
         ...
         and looks.is_published_updated_at > '#{from - 1.day}' and looks.is_published_updated_at <= '#{to}'
       ...
       left outer join (select follow_id, count(*) as count from flinker_follows
         where flinker_follows.updated_at > '#{from - 1.day}' and flinker_follows.updated_at <= '#{to}'
       ...
       left outer join (select flinkers.id as fid, count(*) as count from flinker_likes
        ...
        and flinker_likes.updated_at > '#{from - 1.day}' and flinker_likes.updated_at<= '#{to}'
