class VikingDatatable
  ...
  def fetch_products
    products = Product.where(viking_failure:true).where(merchant_id:Merchant.where("mapping_id is not null").map(&:id)).where("updated_at > ?", 1.day.ago).order("#{sort_column} #{sort_direction}")
