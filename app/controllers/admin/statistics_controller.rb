class Admin::StatisticsController < Admin::AdminController
  ...
  def index
    ...
    @likes = Statistic.top_liked_looks_for_period(Time.now - 1.week, Time.now, 20)
    @top_active = Statistic.top_active_flinkers(Time.now - 1.week, Time.now, 100).includes(:flinker)
  end
